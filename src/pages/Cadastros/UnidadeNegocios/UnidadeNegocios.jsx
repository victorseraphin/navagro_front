...
export default function UnidadeNegociosPage() {
    const [bens, setUnidadeNegocios] = useState(bensIniciais);
    ...
    const handleExcluir = () => {
        ...
        if (window.confirm(`Excluir ${selectedIds.size} item(s)?`)) {
            setUnidadeNegocios((prev) => prev.filter((b) => !selectedIds.has(b.id)));
            setSelectedIds(new Set());
        }
    };
    ...
    return (
        <div className="p-4 px-8 max-w-full mx-auto">
            ...
            <div className="mb-6 border border-gray-300 rounded bg-gray-50 p-4 shadow-sm">
                <h2 className="font-semibold text-xl mb-4 text-gray-700">Unidade de Negócios</h2>
